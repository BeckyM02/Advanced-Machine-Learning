...
# ----------------------------------------------------------
# 5. STREAMLIT APP <--- IMPORTANTE 
# ----------------------------------------------------------

st.title("Lung Segmentation App") #st stands for streamlit
st.write("Carica una RX torace e segmentiamo i polmoni.")


#per nascondere warnings che ci apparirebbero
hide_file_details = """
    <style>
...
uploaded_file = st.file_uploader("Carica un PNG RX", type=["png"]) #caricamente file su streamlit, si deve decidere anche il tipo di file accettato
...
if uploaded_file is not None:
    ...
    with torch.no_grad(): #per mettere il modello in versione di predizione, usiamo no_grad per non calcolare i gradienti in discesa
        y = model(x)

    # Prediction mask (PIL + numpy 0/1)
    pred_mask_img, pred_mask_np = postprocess(y) #necessario da fare il postprocess per ottenere la maschera predetta
    ...
    with colC:
        st.image(overlay, caption="Overlay predizione", use_container_width=True)  #usare metrica che abbiamo usato, tipo abbiamo usato dice loss
